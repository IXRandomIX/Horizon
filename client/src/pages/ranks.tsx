...
import { Trophy, Star, Zap, CheckCircle, Clock, ChevronRight } from "lucide-react";
...
export default function RanksPage() {
  ...
  const currentRank = isStaff
    ? { rank: -1, name: "STAFF", color: "#FFD700" }
    : getRankForXP(xp);
  ...
  const groupedQuests = QUESTS.reduce((acc: Record<string, typeof QUESTS>, q) => {
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-10">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-black" style={{ color: currentRank.color }}>
                {isStaff ? "STAFF" : `Rank ${currentRank.rank === 0 ? "" : currentRank.rank + " · "}${currentRank.name}`}
              </p>
            </div>
        ...
        <div className="space-y-3">
          ...
          <div className="rounded-2xl border border-white/10 overflow-hidden divide-y divide-white/5">
            {[...RANKS].reverse().filter(r => r.rank !== 0).map((r, i) => {
              const isCurrentRank = !isStaff && currentRank.rank === r.rank;
              const achieved = !isStaff && xp >= r.xpNeeded;
              return (
                <div
                  key={r.rank}
                  className={`flex items-center gap-4 px-5 py-4 ${isCurrentRank ? "bg-white/10" : "bg-white/[0.02]"}`}
                >
                  <div
                    className="w-10 h-10 rounded-xl flex items-center justify-center text-lg font-black flex-shrink-0"
                    style={{ background: achieved || isStaff ? `${r.color}22` : "rgba(255,255,255,0.05)", border: `1.5px solid ${achieved || isCurrentRank ? r.color : "rgba(255,255,255,0.08)"}` }}
                  >
        ...
        <div className="space-y-6">
          <h2 className="text-white text-lg font-black uppercase tracking-widest flex items-center gap-2">
            <Zap className="w-5 h-5 text-purple-400" /> Quests
          </h2>
          {Object.entries(groupedQuests).map(([type, quests]) => (
            <div key={type} className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm font-bold text-white/60 uppercase tracking-widest">{QUEST_TYPE_LABELS[type]}</p>
              </div>
              <div className="space-y-2">
                {quests.map(quest => {
                  ...
                  return (
                    <div
                      key={quest.id}
                      className={`rounded-xl border px-4 py-3 flex items-center gap-4 ${completed ? "border-white/10 bg-white/[0.03]" : "border-white/10 bg-white/[0.02]"}`}
                    >
                          ...
                          <div className="mt-1.5 space-y-0.5">
                            <div className="h-1.5 rounded-full bg-white/10 overflow-hidden">
                              <div
                                ...
                                style={{ width: `${pct}%`, background: QUEST_TYPE_COLORS[type] }}
                              />
                            </div>
                      ...
                      <div className="flex-shrink-0 text-right">
                        <span
                          ...
                          style={{ background: completed ? "rgba(255,255,255,0.05)" : `${QUEST_TYPE_COLORS[type]}22`, color: completed ? "rgba(255,255,255,0.2)" : QUEST_TYPE_COLORS[type] }}
                        >
                          +{quest.xp} XP
                        </span>
                      </div>
